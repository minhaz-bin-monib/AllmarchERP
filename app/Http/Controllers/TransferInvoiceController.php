...
class TransferInvoiceController extends Controller
{
     ...
     public function packingListInvoicePdf($transferInvoiceId)
     {
         $numberToWords = new NumberToWords();
         $converter = $numberToWords->getNumberTransformer('en');
         $options = new Options();
        ...
        // $options->set('isRemoteEnabled', true); // Enable remote content
         ...
         if (!is_null($transferInvoice)) {
             ...
             $html = view('templateForPdf.tranferPackingListInvoice')->with($data)->render();
     
             $dompdf->loadHtml($html);
             $dompdf->setPaper('A4', 'portrait');
             $dompdf->render();
             return $dompdf->stream('Packing List Invoice.pdf', ['Attachment' => false]);
         }
