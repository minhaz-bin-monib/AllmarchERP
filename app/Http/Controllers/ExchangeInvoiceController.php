...
*/
   ...
   public function create()
   {
       ...
       $salesInvoice->order_ref = "Exchange";
       ...
       $employeeslist = Employee::where('action_type', '!=', 'DELETE')
                        ...
                        ->orderBy('employee_id')
                        ->get();
       $url = url('/exchangeInvoice/create');
       $toptitle = 'Exchange Invoice';
       $data = compact('salesInvoice', 'employeeslist', 'url', 'toptitle');
       return view('invoice.addExchangeInvoice')->with($data);
   }
   ...
   public function store(Request $request)
   {
       ...
       try {
           ...
           $salesInvoice->invoice_type = 'Exchange';
           $salesInvoice->invoice_type_category = 'Exchange';
           ...
           return redirect('exchangeInvoice/edit/' . $salesInvoice->salesInvoice_id)
   ...
   public function edit($id)
   {
       ...
       if (is_null($salesInvoice)) {
           ...
           $url = url('/exchangeInvoice/update') . "/" . $id;
           $toptitle = 'Exchange Invoice ' . $salesInvoice->salesInvoice_id;
...
*/
   ...
   public function invoiceProductDelete($invoiceId, $invoiceProductid)
   {
       try {
           ...
           // Redirect to the edit page of the newly created invoice
           return redirect('exchangeInvoice/edit/' . $invoiceId)
      ...
      // return redirect('/exchangeInvoice/list');
   }
   // [httpGet]
   ...
   // [httpGet]
   public function exchangeCustomerInvoicePdf($salesInvoiceId)
   {
      ...
      // $options->set('isRemoteEnabled', true); // Enable remote content
       ...
       if (!is_null($salesInvoice)) {
           ...
           $html = view('templateForPdf.exchangeCustomerInvoice')->with($data)->render();
   ...
   public function exchangeDeliveryInvoicePdf($salesInvoiceId)
   {
      ...
      // $options->set('isRemoteEnabled', true); // Enable remote content
       ...
       if (!is_null($salesInvoice)) {
           ...
           $html = view('templateForPdf.exchangeDeliveryInvoice')->with($data)->render();
   ...
   // [httpGet]
   public function exchangeSpecialInvoicePdf($salesInvoiceId)
   {
      ...
      // $options->set('isRemoteEnabled', true); // Enable remote content
       ...
       if (!is_null($salesInvoice)) {
           ...
           $html = view('templateForPdf.exchangeSpecialInvoice')->with($data)->render();
   ...
   public function exchangeSpecialDeliveryInvoicePdf($salesInvoiceId)
   {
      ...
      // $options->set('isRemoteEnabled', true); // Enable remote content
       ...
       if (!is_null($salesInvoice)) {
           ...
           $html = view('templateForPdf.exchangeSpecailDeliveryInvoice')->with($data)->render();
   ...
   // [httpGet]
   public function exchangeSpecialCalculateInvoicePdf($salesInvoiceId)
   {
      ...
      // $options->set('isRemoteEnabled', true); // Enable remote content
       ...
       if (!is_null($salesInvoice)) {
           ...
           $salesInvoiceProduct = DB::table('invoice_products')
               ...
               ->where('invoice_products.salesInvoice_id', '=', $salesInvoice->salesInvoice_id)
               ->where('invoice_products.action_type', '!=', 'DELETE')
               ->select('invoice_products.*', 'batches.import_info', 
                                    'products.product_name', 'products.product_unit_price_c', 'products.atv_rate', 'products.material_description')
               ->get();

           $data = compact('converter','employee', 'salesInvoice', 'salesInvoiceProduct', 'customer'); 

           $html = view('templateForPdf.exchangeSpecialCalculateInvoice')->with($data)->render();
