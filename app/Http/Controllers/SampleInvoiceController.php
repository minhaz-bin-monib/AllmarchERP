   ...
   */
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $salesInvoice->batch_id = $request['batch_id'] ?? '689'; // Assign default if null
            ...
            $salesInvoice->batch_no = $request['batch_no'] ?? ''; // Assign default if null
            ...
            $salesInvoiceProduct->batch_id = $request['batch_id'] ?? '689'; // Assign default if null
            $salesInvoiceProduct->product_id = $request['product_id'];
            $salesInvoiceProduct->manufacturer_id = $request['manufacturer_id'];
            $salesInvoiceProduct->batch_no = $request['batch_no'] ?? ''; // Assign default if null
 ...
 */
    ...
    // [httpGet]
    public function sampleSpecialInvoicePdf($salesInvoiceId)
    {
        ...
        if (!is_null($salesInvoice)) {
           ...
           $data = compact('converter', 'employee','salesInvoice', 'salesInvoiceProduct', 'customer'); 
            ...
            $html = view('templateForPdf.sampleSpecialInvoice')->with($data)->render();
    ...
    public function sampleSpecialDeliveryInvoicePdf($salesInvoiceId)
    {
        ...
        if (!is_null($salesInvoice)) {
           ...
           $data = compact('converter','employee', 'salesInvoice', 'salesInvoiceProduct', 'customer'); 
            ...
            $html = view('templateForPdf.sampleSpecailDeliveryInvoice')->with($data)->render();
    ...
    // [httpGet]
    public function sampleSpecialCalculateInvoicePdf($salesInvoiceId)
    {
        ...
        if (!is_null($salesInvoice)) {
           ...
           $salesInvoiceProduct = DB::table('invoice_products')
            ->join('products', 'invoice_products.product_id', '=', 'products.product_id')
            ->leftJoin('batches', 'batches.batch_id', '=', 'invoice_products.batch_id')
            ->where('invoice_products.salesInvoice_id', '=', $salesInvoice->salesInvoice_id)
            ->where('invoice_products.action_type', '!=', 'DELETE')
            ->select(
                'invoice_products.*',
                DB::raw("CASE 
                            WHEN invoice_products.batch_id = 689 THEN '' 
                            ELSE IFNULL(batches.import_info, '') 
                        END as import_info"),
                'products.product_name',
                'products.product_unit_price_c',
                'products.atv_rate',
                'products.material_description'
            )
            ->get();

           $data = compact('converter','employee', 'salesInvoice', 'salesInvoiceProduct', 'customer'); 

            $html = view('templateForPdf.sampleSpecialCalculateInvoice')->with($data)->render();
